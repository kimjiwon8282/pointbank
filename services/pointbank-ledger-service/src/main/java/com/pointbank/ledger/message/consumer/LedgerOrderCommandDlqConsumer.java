    ...
    private void handle(Message message) {
        try {
            ...
            if (LedgerEventType.BUY_ORDER_REQUESTED.equals(eventType)) {
                dlqHandler.handle(objectMapper.treeToValue(body, BuyOrderRequestedEvent.class));
            } else if (LedgerEventType.SELL_ORDER_REQUESTED.equals(eventType)) {
                dlqHandler.handle(objectMapper.treeToValue(body, SellOrderRequestedEvent.class));
            } else {
