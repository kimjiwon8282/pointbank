...
public class SecuritiesResultDlqConsumer {
    private static final Logger log = LoggerFactory.getLogger(SecuritiesResultDlqConsumer.class);
    ...
    public void consume() {
        for (Message message : sqsClient.receiveMessage(ReceiveMessageRequest.builder()
                .queueUrl(sqsProperties.securitiesResultDlqUrl())
    ...
    private void handle(Message message) {
        try {
            ...
            cleanupService.cleanupCreatedResultDlq(event);
            sqsClient.deleteMessage(DeleteMessageRequest.builder()
                    .queueUrl(sqsProperties.securitiesResultDlqUrl())
                    .receiptHandle(message.receiptHandle())
                    .build());
        } catch (Exception exception) {
            log.warn("Failed to cleanup securities result DLQ message", exception);
        }
    }
}
