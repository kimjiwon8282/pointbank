...
public class BuyOrderAcceptanceService {
    ...
    public BuyOrderResponse accept(Long memberId, String idempotencyKeyValue, BuyOrderRequest request) {
        ...
        long fee = tradingFeePolicy.calculateBuyFee(orderAmount);
        long totalAmount = add(orderAmount, fee);
        SecuritiesOrder order = SecuritiesOrder.builder()
                .orderNo(orderNoGenerator.generateBuyOrderNo())
        ...
        try {
            orderStateService.acceptBuyOrder(order, product.getStockName());
        } catch (DuplicateKeyException exception) {
            SecuritiesOrder concurrentOrder = orderMapper
                    .findByMemberIdAndIdempotencyKey(memberId, idempotencyKey)
                    .orElseThrow(() -> new CustomException(ErrorCode.INTERNAL_SERVER_ERROR));
            return resolveExistingOrder(concurrentOrder, stockCode, request.quantity());
        }
        return toResponse(order, product.getStockName(), "Buy order accepted.");
    }

    private BuyOrderResponse resolveExistingOrder(SecuritiesOrder order, String stockCode, long quantity) {
    ...
    private void validateQuote(String stockCode, QuoteLatestResponse quote) {
        ...
        if (!stockCode.equals(quote.stockCode())
                || quote.currentPrice() <= 0L
                || quote.observedAt() == null) {
            throw new CustomException(ErrorCode.QUOTE_SERVICE_UNAVAILABLE);
        }
    }

    private BuyOrderResponse toResponse(SecuritiesOrder order, String stockName, String message) {
        return new BuyOrderResponse(
                ...
                order.getTotalAmount(), order.getQuoteObservedAt(), message);
    }

    private String normalizeIdempotencyKey(String value) {
        String idempotencyKey = value == null ? null : value.trim();
    ...
    private String normalizeStockCode(String value) {
        String stockCode = value == null ? null : value.trim();
